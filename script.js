...
function finishQuiz() { quizFinished = true; const targetSpread = mobile() ? 5 : 3; spread = targetSpread; render(); const root = mobile() ? rightSlot : rightSlot; const title = root.querySelector('.resultTitle'), txt = root.querySelector('.resultText'), st = root.querySelector('.result-sticker'), rw = root.querySelector('.secretReward'); if (score === 4) { title.textContent = 'Annoyingly perfect.'; txt.textContent = 'You know us too well. I have no secrets left.'; st.textContent = 'dangerously compatible'; rw.textContent = 'one forehead kiss + unlimited cuddles' } else if (score >= 2) { title.textContent = 'You passed.'; txt.textContent = 'Not perfect. Which is good, because neither are we.'; st.textContent = 'approved'; rw.textContent = 'a rematch + one very long hug' } else { title.textContent = 'We need to talk.'; txt.textContent = 'Just kidding. You still pass because I made the rules.'; st.textContent = 'mercy pass'; rw.textContent = 'a second chance and no judgment' } }
